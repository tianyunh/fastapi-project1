...
async def root():
    return {
        "message": "Please enter a date in 2021 with the format 'yyyy-mm-dd' to see how far the next holiday is!"
    }
...
async def daysUntilNextHoliday(date: str):
    ...
    difference = []
    if (date > pd.to_datetime("2021-12-31", format="%Y-%m-%d")) or (
        date < pd.to_datetime("2021-01-01", format="%Y-%m-%d")
    ):
